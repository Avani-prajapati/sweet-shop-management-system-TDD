...
const SweetList = () => {
  ...
  const popularCategories = ["Chocolate", "Candy", "Gum", "Pastry", "Jelly", "Caramel"];
  ...
  const fetchSweets = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/sweets`);
      setSweets(response.data || []);
      
      // Calculate stats
      const totalStock = response.data.reduce((sum, sweet) => sum + sweet.quantity, 0);
      const uniqueCategories = new Set(response.data.map(sweet => sweet.category)).size;
      const totalValue = response.data.reduce((sum, sweet) => sum + (sweet.price * sweet.quantity), 0);
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`${import.meta.env.VITE_API_BASE_URL}/api/sweets/${sweetToDelete}`);
      ...
      setStats(prev => ({
        ...
        categories: new Set(sweets.filter(s => s._id !== sweetToDelete).map(s => s.category)).size,
        totalValue: prev.totalValue - (deletedSweet.price * deletedSweet.quantity)
      }));
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-fuchsia-50 to-rose-50 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="bg-white rounded-2xl p-6 shadow-sm h-32 animate-pulse"></div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {[...Array(8)].map((_, i) => (
            <div key={i} className="bg-white rounded-2xl shadow-sm h-80 animate-pulse"></div>
          ))}
        </div>
      </div>
    );
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-tr from-fuchsia-600 via-pink-500 to-rose-400 p-10 text-white  mb-12">
        ...
        <div className="absolute top-0 right-0 opacity-10 pointer-events-none select-none">
          <svg width="300" height="300" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
            <path fill="#FFFFFF" d="M42.7,-56.3C54.1,-46.4,61.5,-32.2,64.9,-17.3C68.3,-2.4,67.7,13.2,61.2,26.7C54.7,40.2,42.3,51.6,27.5,59.8C12.7,68,-4.5,73.1,-19.5,68.3C-34.6,63.5,-47.5,48.9,-56.2,32.7C-64.9,16.5,-69.4,-1.3,-66.1,-17.4C-62.8,-33.5,-51.7,-47.9,-38.1,-57.1C-24.5,-66.3,-8.4,-70.3,6.5,-78.3C21.4,-86.3,42.8,-98.2,42.7,-56.3Z" transform="translate(100 100)" />
          </svg> 
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        <div className="bg-white rounded-2xl p-6 shadow-lg border border-fuchsia-100 hover:shadow-xl transition-shadow">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-rose-500">Total Products</p>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-lg border border-fuchsia-100 hover:shadow-xl transition-shadow">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-fuchsia-500">Total Stock</p>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-lg border border-fuchsia-100 hover:shadow-xl transition-shadow">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className="text-3xl font-bold text-gray-800">{stats.categories}</h3>
...
{filteredSweets.length === 0 ? (
  <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-fuchsia-100 text-center p-12">
    ...
    <h3 className="text-2xl font-semibold text-gray-800 mb-3">No sweets found</h3>
    <p className="text-gray-500 mb-6">Try adjusting your filters or add a new sweet to your inventory</p>
    <Link 
      to="/sweets/add" 
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
    {filteredSweets.map((sweet) => (
      ...
      >
        ...
        <div className={`h-2 bg-gradient-to-r ${getCategoryColor(sweet.category)}`}></div>
        ...
        <div className="p-4 flex flex-col flex-grow">
          ...
          <div className="space-y-3 mb-4">
            ...
            <div className="flex justify-between items-center bg-green-50 rounded-lg p-2">
              ...
              <span className={`text-xs font-bold px-2 py-1 rounded-lg ${sweet.quantity < 10 ? "bg-red-100 text-red-800" : "bg-green-100 text-green-800"}`}>
        ...
        <div className="fixed inset-0 bg-transparent bg-opacity-50 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-gray-200">
            <div className="p-8 space-y-6 text-center">
              ...
              <h3 className="text-2xl font-bold text-gray-800">Delete Sweet?</h3>
              <p className="text-gray-500">
                Are you sure you want to delete <span className="font-semibold text-gray-800">"{sweets.find(s => s._id === sweetToDelete)?.name}"</span>? This action cannot be undone.
              </p>
